class StoriesController < ApplicationController
  ...
  def index
    @stories = @project.stories.by_date
    ...
    respond_with @project, @stories
  end

  def show
    @story = @project.stories.find(params[:id]).decorate
    respond_with @project, @story
  end
  ...
  def create
    ...
    @story.epic = @epic if epic?
    flash[:notice] = '@project.stories created' if @story.save
    respond_with @project, @story
  end
  ...
  def update
    ...
    if @story.update_attributes(params[:story].merge(updated_by: current_user))
      flash[:notice] = '@project.stories updated'
    end
    respond_with @project, @story
  end
